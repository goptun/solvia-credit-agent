...
**Full-text search: OR over parsed lexemes** (fixed post-baseline, before merge — real finding, see PR #3's before/after table): the full-text query originally built its `tsquery` with `plainto_tsquery`, which ANDs every term in the input together. That works for a short, targeted question but silently drops a long, colloquial one the moment it shares only some of its vocabulary with the target chunk — exactly the phrasing style the eval set's `colloquial` questions are designed to exercise, and exactly why their full-text contribution to recall was near zero in the recorded baseline. The fix parses the query text into `portuguese_unaccent` lexemes with `to_tsvector`, then rebuilds a `tsquery` by OR-joining them (`to_tsquery('portuguese_unaccent', array_to_string(tsvector_to_array(to_tsvector(...)), ' | '))`) before running it against `content_tsv` — `ts_rank_cd` still orders by how many/how-weighted the matched lexemes are, so a chunk matching more of the question's vocabulary still ranks above one matching only one word; a chunk that tokenizes to zero lexemes (rare — stop-words-only input) yields an empty `tsquery` that matches nothing, not a crash and not every row.

Measured on the real corpus (rerun of task 6.4's eval, same 25 answerable + 6 unanswerable questions, only this query changed): recall@k 64.00% → 68.00% overall, and — the number this fix specifically targets — 33.33% → **50.00%** on `colloquial` questions, with `lexical` unchanged (73.68%). Refusal accuracy also rose, 50.00% → 83.33% (`far` 100%, `near_miss` 75%), because the single top-1 fused chunk `evaluate_unanswerable` inspects can itself change once FTS surfaces more competitors. The honest trade-off: MRR fell, 0.523 → 0.438 overall (0.623 → 0.480 on `lexical`) — RRF fuses by **rank position only**, not `ts_rank_cd`'s score magnitude, so once several chunks satisfy the looser OR query, a lexically-exact top hit that used to be the *only* full-text match (and so always ranked first) now merely ranks *high*, and an unrelated chunk's essentially-arbitrary vector rank (`FakeEmbeddings` in tests; the real embedding model in this eval) can occasionally out-fuse it into rank 2. Kept as specified rather than tuned further — recall is the metric an OR rewrite is meant to move, and a full reranking layer (already an extension point — `RerankerPort`, disabled by default) is the documented way to recover top-1 precision without giving back the recall gain, not a change to make silently here.
...
**Benchmark methodology and safety** (real incident, pre-merge review — task 14.2): a first attempt to benchmark `intfloat/multilingual-e5-large` locally — one Python process, `fastembed` defaults (`threads=None`, i.e. `onnxruntime` auto-detects and uses every core; `TextEmbedding.embed(batch_size=256)`), embedding an entire document's chunks in a single call — exhausted the development machine's RAM (~9 GiB) and froze it. Two production-code fixes came out of this, not just a benchmark-script workaround: `FastEmbedAdapter` now always constructs `TextEmbedding(threads=...)` (default `RagSettings.rag_embedding_threads = 2`, matching the target VPS's 2 vCPUs, never `onnxruntime`'s all-cores autodetection) and always calls `.embed(..., batch_size=..., parallel=None)` with a capped `rag_embedding_batch_size = 16` for passage/document embedding (`embed_query` always uses `batch_size=1`, a single string). Every caller goes through this one adapter, so `rag.ingest index` — the only place that ever embeds a whole document's chunks at once — inherits the cap automatically; there is no separate "benchmark-safe" code path to keep in sync. The actual model comparison was then re-run inside a `docker run --memory=3g --cpus=2` container (a compute/RAM budget close to the VPS, not the full local Docker Desktop allocation) talking to the existing local Postgres over the network, one document indexed at a time — see the comparison table below.
...
The same run doubles as the practical calibration exercise for `RAG_MIN_RELEVANCE_SCORE`. Done for real while implementing this (not left as an aspiration): a direct sweep of every eval question's best retrieved similarity showed the answerable and unanswerable distributions genuinely **overlap** (answerable best-similarity ranged ~0.50–0.85; unanswerable ranged ~0.52–0.65) — no threshold separates them perfectly with this embedding model. The naive first default (`0.75`, picked before this data existed) answered only 28% of answerable questions; `0.6` answers 88% of them while still refusing half the unanswerable ones — a materially better trade-off, and the value `RagSettings.rag_min_relevance_score` now defaults to. This is also what caught a real bug during the manual smoke test (task 9.3): the old `0.75` default silently refused the smoke test's own `product_question` turn (a straightforward, real question the corpus does cover), which recall@k/MRR alone — both computed independently of this threshold — would never have surfaced.
...
New `rag/` top-level package (sibling to `apps/`, mirroring the project's existing ports-and-adapters style): `rag/corpus/` (manifest schema + loader), `rag/ingest/` (`html_source.py`/`pdf_source.py` format adapters, `chunking.py`, `fetch.py`, indexing CLI), `rag/embeddings/` (port + fastembed/fake adapters), `rag/retrieval/` (hybrid search, RRF, reranker port, the `hybrid_search_async` live wrapper), `rag/eval/` (question set + recall@k/MRR/refusal-accuracy CLI). Everything in `rag/` itself has no dependency on `apps/`, so it stays reusable/testable independent of the conversation graph, matching the existing rule that domain logic (here, retrieval) has no LangChain/FastAPI imports beyond what `EmbeddingsPort`/adapters need.

Four places import from both `apps/` and `rag/` — more than originally planned, and worth naming precisely: `apps/agent/nodes/knowledge_agent.py` (`ground_answer`, the shared grounding/citation core, plus the graph-node wrapper around it; kept testable without Postgres via the injected `RetrieveFn` type, itself defined in `apps/`, not `rag/`), `apps/api/routes_knowledge.py` (the standalone endpoint, task 9.3, calling the same `ground_answer`), and the app's composition layer — `apps/api/app.py`, where a `psycopg_pool.ConnectionPool`, `FastEmbedAdapter`, and `RagSettings` actually get constructed and wired into `knowledge_agent_node`'s `RetrieveFn`, and `apps/api/context.py`, whose `AppContext` dataclass carries those same built objects (`EmbeddingsPort`, `RetrieveFn`, `RagSettings`) so `routes_knowledge.py` can reuse them per request instead of rebuilding them. A composition root (and the plain data container it populates) importing every layer it wires together is the expected shape of that pattern, not a boundary violation — the rule this section protects is that `rag/`'s own code, and the *business logic* inside `knowledge_agent.py`, never depend on `apps/`, which still holds.
