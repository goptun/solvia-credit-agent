...
Candidates evaluated for the `fastembed` adapter, given the target VPS is a 2 vCPU ARM host already running several other containers (per `docs/infra-assessment.md`):

| Model | Dim | Approx. size | Portuguese coverage | Note |
|---|---|---|---|---|
| `intfloat/multilingual-e5-small` | 384 | ~470 MB (fp32) | Explicitly multilingual, incl. pt | Smallest of the credible multilingual options; smaller index (384-dim `VECTOR` column above) |
| `intfloat/multilingual-e5-large` | 1024 | ~2.2 GB | Best quality of the three | Too heavy for a shared 2 vCPU ARM host — rejected on footprint alone |
| `granite-embedding-278m-multilingual` | 768 | ~1.1 GB | Explicit pt support, IBM-trained | Good quality/size middle ground; candidate if `multilingual-e5-small`'s recall@k is insufficient |

Default choice: **`intfloat/multilingual-e5-small`** — smallest footprint, explicit multilingual training, and the corpus here is small and domain-narrow (legal Portuguese, not open-domain), where a small model's main risk (missing subtle semantic distinctions) matters less than on an open-domain benchmark. The retrieval-quality eval (recall@k/MRR over the committed question set) is the actual gate: the apply-phase ADR records the measured memory footprint (`docker stats` on a comparable container) and the eval numbers, and upgrades to `granite-embedding-278m-multilingual` (updating `VECTOR(384)` to `VECTOR(768)` in the migration) if recall@k falls short. This is a single-parameter swap (model name + vector dimension), not a retrieval-architecture change, so deferring the final pick to measured numbers is safe.

### Retrieval quality evaluation

`rag/eval/questions.yaml` has two parts:
- **~24 answerable questions** in Portuguese, spanning all four regulatory documents and the product catalog, each with an expected `document_id` + `article_ref`. Every question is **paraphrased** — written the way a customer would actually ask, not copied verbatim from the article's own sentence — so the eval measures semantic retrieval, not substring matching.
- **5–8 deliberately unanswerable questions** (plausible-sounding but outside the corpus's actual coverage, or about a topic the corpus doesn't address), each expecting a refusal rather than an expected document/article.

`rag/eval/run.py` reports three numbers: **recall@k** and **MRR** over the answerable subset (whether/where the expected document+article appears in the retrieved results), and **refusal accuracy** over the unanswerable subset (the fraction correctly triggering the cosine-similarity refusal threshold from "Grounding and citation validation" above) — the same run also doubles as the practical calibration exercise for `RAG_MIN_RELEVANCE_SCORE`: the threshold is picked as the value that maximizes refusal accuracy on the unanswerable subset without materially hurting recall@k on the answerable one.

Before the baseline run is recorded (for the ADR and the PR description), a 10-question sample of the eval set (including at least two unanswerable questions) is presented for review — catching a bad or ambiguous question before it skews the recorded baseline is cheaper than re-baselining after the fact.

### Grounding and citation validation

`apps/agent/nodes/knowledge_agent.py` (smart tier, added to `NODE_TIER_MAP`) is the only place that turns retrieved chunks into a customer-facing answer:
