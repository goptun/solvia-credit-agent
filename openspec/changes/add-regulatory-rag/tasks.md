...
- [ ] 3.1 Add `lxml`/`beautifulsoup4` and `pdfplumber` to `pyproject.toml` dependencies — verify `uv run python -c "import bs4, pdfplumber"` succeeds
- [ ] 3.2 Implement `rag/ingest/html_source.py`: decode using the source page's declared encoding (not an assumed UTF-8), drop `<strike>`/`<s>`/`<del>` element text before extracting body text, and extract trailing amendment/revocation notes into a separate `amendment_note` field per article instead of leaving them in body text, per `design.md` — "Source format handling" — verify unit tests against a real snippet copied from the fetched CDC page containing both a revoked provision and an amended one assert: the revoked provision's text never appears in the adapter's output, and the amendment note appears only in `amendment_note` metadata, never concatenated into body text
- [ ] 3.3 Implement `rag/ingest/pdf_source.py` using `pdfplumber` (see `design.md` for the pypdf-vs-pdfplumber justification): position-aware repeated header/footer removal and hyphenated line-break rejoining — verify a unit test against a real page snippet from one of the fetched BCB PDFs asserts the repeated header/footer text is absent from the extracted text and a hyphenated word is rejoined correctly
- [ ] 3.4 Commit: `feat(rag): adiciona extração de HTML e PDF com remoção de texto revogado e artefatos de formatação`
- [ ] 3.5 Implement `rag/ingest/chunking.py`: structure-aware splitting (Capítulo/Seção/Art./§/inciso/alínea) of the normalized text produced by the source-format adapters, keeping an in-budget article whole and splitting an oversized article by paragraph with a repeated article header, per `design.md` — "Chunking strategy" — verify unit tests using the same real snippets from 3.2/3.3 cover: a whole in-budget article, an oversized article split with repeated headers, and a product-catalog entry
- [ ] 3.6 Attach chunk metadata (`document_id`, `norm`, `article_ref`, `hierarchy_path`, `source_url`, `version_date`, `source_type`, `amendment_note`) to every chunk — verify a unit test asserts all fields are populated for both a regulatory chunk and a product-catalog chunk, and that `amendment_note` (when present) is never a substring of `content`
- [ ] 3.7 Commit: `feat(rag): implementa chunking sensível à estrutura de textos legais`
...
- [ ] 5.1 Define `EmbeddingsPort` (`rag/embeddings/port.py`) with `embed_query`/`embed_documents` methods and a `FakeEmbeddings` adapter (deterministic, hash-based vectors, no network/model download) — verify a unit test asserts identical input text always yields an identical vector via either method
- [ ] 5.2 Implement `FastEmbedAdapter` using `intfloat/multilingual-e5-small` (see `design.md` — "Embedding model trade-offs"), applying the `"query: "` prefix inside `embed_query` and `"passage: "` inside `embed_documents` — verify a unit test (skipped in CI via a marker, run locally) asserts the text actually sent to the underlying model is prefixed correctly for each method, and that a short Portuguese sentence embeds to a 384-dim vector
- [ ] 5.3 Implement `rag/retrieval/hybrid.py`: vector similarity query + full-text query (against the `portuguese_unaccent`-backed column) + Reciprocal Rank Fusion, an optional `source_type` filter applied to both queries, and returning each result's raw vector cosine similarity and `matched_fts` flag alongside the fused rank, with `k_vector`/`k_fts`/`rrf_k`/`top_k` read from settings — verify an integration test (pgvector service container, `FakeEmbeddings`) asserts: the fused ranking differs from either individual ranking on a case constructed to need both signals; a `source_type` filter excludes the other type entirely; and the returned raw similarity is independent of fused rank (a lower-fused-rank result can carry a higher raw similarity than one ranked above it)
- [ ] 5.4 Define `RerankerPort` and a `NoopReranker` default; wire `RAG_RERANKING_ENABLED` to select it — verify a unit test asserts `hybrid.py` calls the reranker unconditionally and `NoopReranker` is a pass-through
- [ ] 5.5 Commit: `feat(rag): implementa embeddings, busca híbrida e fusão RRF`
