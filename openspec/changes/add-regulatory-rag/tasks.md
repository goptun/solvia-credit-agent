...
- [x] 6.1 Write a committed Portuguese evaluation set (`rag/eval/questions.yaml`): 25 paraphrased (not copied from article text) answerable questions covering all four regulatory documents and the product catalog, each with `expected_refs` (a list, since some questions have more than one legitimate answer), a verified `evidence` quote, and a `style` (lexical/colloquial), plus 6 deliberately unanswerable questions tagged `distance` (far/near_miss) expecting a refusal — verify the file parses against a Pydantic schema in unit tests that also assert evidence is a real substring of the cited article and no answerable question's text is a substring of its cited article's stored content
- [x] 6.2 Present a random 10-question sample from the set (including at least 2 unanswerable questions) to me for review — pause here; do not proceed to 6.4 until I confirm the sample looks correct (revised after review: added `evidence`/`expected_refs`/`style`/`distance`, rebalanced toward CDC arts. 54-A–G and art. 52 and toward CET disclosure, replaced 3 easy negatives with near-misses, rephrased 6 questions colloquially with 3 unaccented)
- [x] 6.3 Implement `rag/eval/run.py` reporting recall@k and MRR (broken down by `style`) over the answerable questions plus refusal accuracy (broken down by `distance`) over the unanswerable ones — verify it runs against a locally-ingested corpus and prints all three metrics; also added `python -m rag.ingest index` (chunk + embed + index every manifest document), needed to actually populate a corpus to run this against — no earlier task built it
- [x] 6.4 Run `rag/eval/run.py` against the ingested corpus and record the baseline numbers to paste into the PR description (task 13.1) — **baseline (real `paraphrase-multilingual-MiniLM-L12-v2` embeddings, real corpus, `top_k=5`)**: recall@k 64.00% overall (lexical 73.68%, colloquial 33.33%), MRR 0.523 overall (lexical 0.623, colloquial 0.208), refusal accuracy 100% overall (far 100%, near_miss 100%, n=2/4). Colloquial recall is markedly weaker than lexical — the realistic-question signal the eval was built to surface — informs the embedding-model ADR (task 9.1)
- [x] 6.5 Commit: `test(rag): adiciona conjunto de avaliação e CLI de recall@k/MRR/acurácia de recusa`
