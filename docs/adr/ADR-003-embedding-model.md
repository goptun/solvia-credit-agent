...
- **MiniLM's similarity ranges overlap** (answerable 0.47–0.85, unanswerable 0.43–0.63). No `RAG_MIN_RELEVANCE_SCORE` cleanly separates them, so the threshold (0.6) is only a **coarse first filter** that drops clearly irrelevant retrievals cheaply. Actual refusal must rely on the LLM grounding stage (measured end to end, with its latency and smart-vs-fast comparison, in `ADR-005-llm-latency-and-timeouts.md`) — the `knowledge_agent` returning an empty `claims` list when the retrieved chunks do not answer the question (task 14.3), and citation validation dropping ungrounded claims.
